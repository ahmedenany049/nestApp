...
import {
  ...
  logInDto,
  reSendOtpDto,
} from './dto/add_user.dto';
import { userDocument, UserRepo } from 'src/DB';
...
export class UserService {
  ...
  async confirmEmail(body: confirmEmailDto) {
    ...
    if (
      !(await Compare({ plainText: code, hash: (user.otp as any)?.[0].code }))
    ) {
  ...
  uploadFile(file: Express.Multer.File, user: userDocument) {
